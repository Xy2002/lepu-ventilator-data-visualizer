...
pressure / real_pres 图表的 y 轴取 0.5%/99.5% 分位数范围并外扩 8%（下限不低于 0）。治疗启动自检过冲、咳嗽等瞬时尖峰是真实测量值（实测可达 40 cmH₂O），但持续时间短，若按数据实际范围缩放会独占刻度、把治疗频带（EPAP/IPAP）压扁，故超出分位范围的样本裁剪在网格外；flow 等波形通道的形状本身是信号，仍按数据实际范围显示。
