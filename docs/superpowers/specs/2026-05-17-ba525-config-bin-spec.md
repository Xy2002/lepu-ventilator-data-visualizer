# 乐普 BA525 呼吸机配置二进制格式规范

**设备型号**：乐普（Lepu）BA525
**日期**：2026-05-17 ~ 2026-05-18
**状态**：所有 UI 可调参数已全部 diff-verified；其余字节保留供后续探索

> 本文档描述从乐普 BA525 呼吸机 UI **导出的配置二进制**（192 字节）的格式。已通过对 12 个连续导出样本（user 命名 `config_v1.bin` ~ `config_v12.bin`，仅为顺序编号，不是格式版本）的字节级 diff 反向工程而得。
>
> 实现：`src/parser/ba525ConfigParser.ts`（零依赖 TypeScript 解析器）。

---

## 1. 概览

BA525 配置二进制是一段固定 **192 字节** 的 payload，由用户从呼吸机 UI 导出。文件本身**不含 EDF header、不含时间戳**——纯粹是 UI 配置项的二进制快照。

与项目内已有的 `docs/config_edf_analysis.md` 分析的 EDF `config` 区别：

| 维度 | `config.edf` payload（治疗会话记录里的 config 块） | BA525 配置导出（本规范） |
|---|---|---|
...
**关键观察**：这个 192 字节的 BA525 配置 bin 实际上就是 EDF config 中"配置数据"部分本身（无时间戳）。EDF 旧分析中提到的 6 区结构在本文件中同样成立（旧文档文字写"5 区"但实际列出 6 区，本文档统一作 6 区）。
