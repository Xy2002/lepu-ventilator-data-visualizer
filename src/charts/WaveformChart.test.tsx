  ...
  it("initializes ECharts with the canvas renderer and sample metadata", () => {
    ...
    render(
      <WaveformChart
        ...
        eventMarkers={[
          { timestamp: "2026-04-29 03:03:13.57", sourceLabel: "ai" },
        ]}
      />
    );
    ...
    expect(echartsCoreMock.init).toHaveBeenCalledWith(
  ...
  it("centers the ECharts dataZoom around a focused event second", () => {
    ...
    render(
      <WaveformChart
        label="flow"
        values={
          new Uint8Array(Array.from({ length: 400 }, (_, index) => index % 255))
        }
        sampleRateHz={80}
        focusedSecond={2}
      />
  ...
  it("centers the ECharts dataZoom around a focused EDF timestamp when real time is available", () => {
    ...
    render(
      <WaveformChart
        ...
        values={
          new Uint8Array(
            Array.from({ length: 1600 }, (_, index) => index % 255)
          )
        }
        sampleRateHz={80}
        startTime="2026-04-29 03:03:12.57"
    ...
    expect(
      screen.getByText("焦点：2026-04-29 03:03:14.57")
    ).toBeInTheDocument();
