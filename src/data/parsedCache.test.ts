...
describe("parsedCache", () => {
  ...
  describe("buildManifest", () => {
    it("sorts files by path and extracts metadata", () => {
      const files = [
        fileRef("b/path.edf", 200, 2000),
        fileRef("a/path.edf", 100, 1000),
      ];
  ...
  describe("manifestMatches", () => {
    ...
    it("returns false when file count differs", () => {
      ...
      const manifest = buildManifest([
        fileRef("a.edf", 100, 1000),
        fileRef("b.edf", 200, 2000),
      ]);
  ...
  describe("saveParsedDataset / loadParsedDataset", () => {
    ...
    it("returns null when manifest does not match", async () => {
      ...
      const changedFiles = [
        fileRef("DATAFILE/20260428/20260428_flow.edf", 515, 9999),
      ];
    ...
    it("overwrites previous cache on save", async () => {
      ...
      const newIndex = makeIndex({
        ...
        summariesByDay: {
          "2026-04-29": {
            ...base.summariesByDay["2026-04-28"],
            date: "2026-04-29",
          },
        },
    ...
    it("preserves Uint16Array and Int16Array typed arrays", async () => {
      const files = [
        fileRef("DATAFILE/20260428/20260428_pressure.edf", 516, 1000),
      ];
      const index = makeIndex({
        parsedFilesByDay: {
          "2026-04-28": [
    ...
    it("reconstructs filesByDay from loaded files", async () => {
      ...
      const index = makeIndex({
        ...
        summariesByDay: {
          ...
          "2026-04-29": {
            ...makeIndex().summariesByDay["2026-04-28"],
            date: "2026-04-29",
          },
        },
      });
      ...
      expect(Object.keys(loaded!.filesByDay).sort()).toEqual([
        "2026-04-28",
        "2026-04-29",
      ]);
